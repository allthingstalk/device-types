function converter(code) {
    function decodeUplink(input) {
        var decoded = {};
        var att = {};
        decoded.index = decode_index(input);
        att.index = { value: decoded.index/10 };
        decoded.step = decode_step(input);
        att.step = { value: decoded.step };
        decoded.list_increment = decode_list_increment(input);
        // att.list_increment = WIP
    ...
    return JSON.stringify(decodeUplink(code));
}