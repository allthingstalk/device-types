            ...
            if (decodedBatch === false) {
                ...
                if ((cmdID === 0x0a) | (cmdID === 0x8a) | (cmdID === 0x01)) {
                    ...
                    if ((clusterdID === 0x0052) & (attributID === 0x0000)) {
                        ...
                        att.ActiveEnergyWh = { value: UintToInt(bytes[index + 1] * 256 * 256 + bytes[index + 2] * 256 + bytes[index + 3], 3) / 1000 };
                        ...
                        att.ReActiveEnergyVARh = { value: UintToInt(bytes[index + 4] * 256 * 256 + bytes[index + 5] * 256 + bytes[index + 6], 3) / 1000 };
                        tab.push({ label: "NumberOfSample", value: (bytes[index + 7] * 256 + bytes[index + 8]) / 1000, date: lDate });
                        att.NumberOfSample = { value: (bytes[index + 7] * 256 + bytes[index + 8]) / 1000 };
                        tab.push({ label: "ActivePowerW", value: UintToInt(bytes[index + 9] * 256 + bytes[index + 10], 2) / 1000, date: lDate });
                        att.ActivePowerW = { value: UintToInt(bytes[index + 9] * 256 + bytes[index + 10], 2) / 1000 };
                        tab.push({ label: "ReActivePowerVAR", value: UintToInt(bytes[index + 11] * 256 + bytes[index + 12], 2) / 1000, date: lDate });
                        att.ReActivePowerVAR = { value: UintToInt(bytes[index + 11] * 256 + bytes[index + 12], 2) / 1000 }
                    }
                ...
                // TODO: Implement sending arrays to platform so multiple datapoints can be sent at once
